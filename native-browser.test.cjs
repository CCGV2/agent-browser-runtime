...
test('real MCP: approval, page origins, embedded content, no replay, revocation and auditing', {timeout:150_000,skip:!process.env.AGENT_BROWSER_TEST_EXECUTABLE}, async t => {
  ...
  const firstTabs = await store.viewer.request({session:liveSession,action:{type:'tabs'}});
  const firstTabID = firstTabs.activeTabID;
  assert.ok(firstTabID);
  assert.ok(!(await call('browser_tabs',{action:'new',url:origin+'/iframe'})).isError);
  const laterTabs = await store.viewer.request({session:liveSession,action:{type:'tabs'}});
  assert.notEqual(laterTabs.activeTabID,firstTabID);
  const pinnedFrame = await store.viewer.request({session:liveSession,action:{type:'screenshot',tabID:firstTabID}});
  assert.ok(pinnedFrame.image,JSON.stringify(pinnedFrame));
  assert.equal((await store.viewer.request({session:liveSession,action:{type:'tabs'}})).activeTabID,laterTabs.activeTabID);
  assert.ok(!(await call('browser_tabs',{action:'close'})).isError);
  assert.ok(!(await call('browser_click',{target:'#count'})).isError);
  ...
  assert.equal(preview.pointer?.action, 'click');
  assert.ok(Number.isFinite(preview.pointer.x));
